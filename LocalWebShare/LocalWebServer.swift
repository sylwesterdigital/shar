    ...
    private func sendHTML() {
        ...
        <body data-button-mode="icons" data-view="grid" data-theme="ocean">
          ...
          <script>
            ...
            async function remoteAPI(path,opt={}){const r=await fetch(REMOTE_API+path,{cache:'no-store',...opt,headers:{'Content-Type':'application/json','X-Shar-Client':'browser-sender-2.1.2',...(opt.headers||{})}});let j={};try{j=await r.json()}catch{}if(!r.ok)throw Error(j.error||`HTTP ${r.status}`);return j}
