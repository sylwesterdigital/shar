...
private struct MacDeveloperUpdatesView: View {
    ...
    private let updates: [(String, String, String, String)] = [
        ("2.2.445", "2026-08-31", "iOS Debug-dylib linkage verification fix", "Fixes the Xcode 16 Debug-build launch-contract verifier to inspect LocalWebShare.debug.dylib when it owns the weak Whisper load command, while Release builds continue to verify the main executable."),
        ("2.2.444", "2026-08-31", "Android Whisper packaging memory fix", "Stops AAPT2 from compressing the ~148 MB local Whisper model, raises controlled Gradle/Kotlin heap limits, and keeps the existing persistent Whisper cache plus resilient Apple notarization flow."),
        ("2.2.443", "2026-08-31", "Persistent Whisper cache + network recovery", "Keeps validated Whisper dependencies across watcher updates, resumes interrupted downloads, fixes the zsh notarization-state variable crash, and retries Developer ID timestamp/notary network operations."),
        ("2.2.442", "2026-08-31", "Notary recovery + watcher quarantine", "macOS notarization now submits once and polls the accepted Apple submission through transient network outages; the watcher also quarantines malformed stable ZIP signatures instead of retrying them every poll."),
        ("2.2.44", "2026-08-31", "Apple launch crash + Whisper isolation", "Weak-linked the local Whisper runtime and added runtime availability checks so the app launches independently of the optional caption framework; iOS also gets the required Frameworks runpath."),
