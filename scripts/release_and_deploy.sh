...
network_retry(){ local n=1 max="${SHAR_NETWORK_ATTEMPTS:-20}" delay="${SHAR_NETWORK_RETRY_DELAY:-10}"; until "$@"; do local rc=$?; (( n >= max )) && return "$rc"; shar_warn "Network operation failed; retry $n/$max in ${delay}s: $*"; sleep "$delay"; n=$((n+1)); done; }
...
network_retry gh auth status -h github.com >/dev/null 2>&1 || fail "GitHub CLI authentication/network check failed after retries."
...
network_retry ./scripts/check_remote_share.sh
source ./scripts/release_profile.sh
shar_load_release_profile
network_retry ssh -o BatchMode=yes -o ConnectTimeout=12 -p "$SHAR_REMOTE_PORT" "$SHAR_REMOTE_USER@$SHAR_REMOTE_HOST" true || fail "Homepage SSH access failed after retries."
