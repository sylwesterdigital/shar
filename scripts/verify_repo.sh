...
grep -Fq '2.2.34", "2026-08-31", "Audio continuity + visual captions' macos/LocalWebShareMacApp.swift || fail "macOS dated v2.2.34 developer update missing"
grep -Fq 'version: "2.2.33", date: "2026-08-31"' LocalWebShare/ContentView.swift || fail "iOS dated v2.2.33 developer update missing"
grep -Fq 'version: "2.2.34", date: "2026-08-31"' LocalWebShare/ContentView.swift || fail "iOS dated v2.2.34 developer update missing"
grep -Fq '2.2.35", "2026-08-31", "Live spectrum + resilient captions' macos/LocalWebShareMacApp.swift || fail "macOS dated v2.2.35 developer update missing"
...
grep -Fq '2.2.442", "2026-08-31", "Notary recovery + watcher quarantine' macos/LocalWebShareMacApp.swift || fail "macOS dated v2.2.442 developer update missing"
grep -Fq 'version: "2.2.442", date: "2026-08-31", title: "Notary recovery + watcher quarantine"' LocalWebShare/ContentView.swift || fail "iOS dated v2.2.442 developer update missing"
grep -Fq 'v2.2.442 · 2026-08-31  Notary recovery + watcher quarantine' android/app/src/main/java/com/localwebshare/app/MainActivity.java || fail "Android dated v2.2.442 developer update missing"
grep -Fq '2.2.443", "2026-08-31", "Persistent Whisper cache + network recovery' macos/LocalWebShareMacApp.swift || fail "macOS dated v2.2.443 developer update missing"
grep -Fq 'version: "2.2.443", date: "2026-08-31", title: "Persistent Whisper cache + network recovery"' LocalWebShare/ContentView.swift || fail "iOS dated v2.2.443 developer update missing"
grep -Fq 'v2.2.443 · 2026-08-31  Persistent Whisper cache + network recovery' android/app/src/main/java/com/localwebshare/app/MainActivity.java || fail "Android dated v2.2.443 developer update missing"
grep -Fq '2.2.445", "2026-08-31", "iOS Debug-dylib linkage verification fix' macos/LocalWebShareMacApp.swift || fail "macOS dated v2.2.445 developer update missing"
grep -Fq 'version: "2.2.445", date: "2026-08-31", title: "iOS Debug-dylib linkage verification fix"' LocalWebShare/ContentView.swift || fail "iOS dated v2.2.445 developer update missing"
grep -Fq 'v2.2.445 · 2026-08-31  iOS Debug-dylib linkage verification fix' android/app/src/main/java/com/localwebshare/app/MainActivity.java || fail "Android dated v2.2.445 developer update missing"
grep -Fq '2.2.444", "2026-08-31", "Android Whisper packaging memory fix' macos/LocalWebShareMacApp.swift || fail "macOS dated v2.2.444 developer update missing"
grep -Fq 'version: "2.2.444", date: "2026-08-31", title: "Android Whisper packaging memory fix"' LocalWebShare/ContentView.swift || fail "iOS dated v2.2.444 developer update missing"
grep -Fq 'v2.2.444 · 2026-08-31  Android Whisper packaging memory fix' android/app/src/main/java/com/localwebshare/app/MainActivity.java || fail "Android dated v2.2.444 developer update missing"
grep -Fq "noCompress += ['bin']" android/app/build.gradle || fail "Android Whisper model is not excluded from AAPT compression"
grep -Fq 'org.gradle.jvmargs=-Xmx4096m' android/gradle.properties || fail "Android Gradle release heap headroom missing"
grep -Fq 'kotlin.daemon.jvmargs=-Xmx2048m' android/gradle.properties || fail "Android Kotlin daemon heap headroom missing"
grep -Fq 'org.gradle.workers.max=2' android/gradle.properties || fail "Android worker cap missing for large-model packaging"
grep -Fq '2.2.44", "2026-08-31", "Apple launch crash + Whisper isolation' macos/LocalWebShareMacApp.swift || fail "macOS dated v2.2.44 developer update missing"
grep -Fq 'version: "2.2.44", date: "2026-08-31", title: "Apple launch crash + Whisper isolation"' LocalWebShare/ContentView.swift || fail "iOS dated v2.2.44 developer update missing"
...
grep -Fq 'LD_RUNPATH_SEARCH_PATHS = "$(inherited) @executable_path/Frameworks";' LocalWebShare.xcodeproj/project.pbxproj || fail "iOS embedded-framework runtime search path missing"
grep -Fq 'ATTRIBUTES = (Weak, );' LocalWebShare.xcodeproj/project.pbxproj || fail "iOS whisper.xcframework is not weak-linked"
grep -Fq 'shar_whisper_runtime_available' LocalWebShare/LocalWhisperBridge.c LocalWebShare/LocalMediaIntelligence.swift || fail "Apple local Whisper runtime availability guard missing"
grep -Fq 'dlsym(RTLD_DEFAULT' LocalWebShare/LocalWhisperBridge.c || fail "Whisper runtime-symbol probe missing"
grep -Fq -- '-Xlinker -weak_framework -Xlinker whisper' scripts/build_macos_release.sh scripts/build_macos.sh || fail "macOS Whisper is not weak-linked"
[[ -x scripts/verify_ios_whisper_linkage.sh ]] || fail "shared iOS Whisper linkage verifier missing or not executable"
grep -Fq 'LocalWebShare.debug.dylib' scripts/verify_ios_whisper_linkage.sh || fail "iOS verifier does not understand Xcode 16 Debug dylib mode"
grep -Fq 'LC_LOAD_WEAK_DYLIB' scripts/verify_ios_whisper_linkage.sh || fail "iOS verifier does not require a weak Whisper load command"
grep -Fq 'verify_ios_whisper_linkage.sh' scripts/app_build.sh scripts/build_ios_check.sh || fail "iOS build scripts do not use the shared Whisper linkage verifier"
grep -Fq 'iOS embedded Whisper launch contract verified' scripts/verify_ios_whisper_linkage.sh || fail "iOS post-build Whisper launch-contract verification missing"
...
grep -Fq 'grep -Fqx -- "$signature" "$STATE_FILE"' scripts/build-watch.sh || fail "build watcher processed-signature ledger lookup missing"
grep -Fq 'tail -n 256 "$STATE_FILE"' scripts/build-watch.sh || fail "build watcher bounded processed-signature ledger missing"
grep -Fq 'mark_processed "$signature"' scripts/build-watch.sh || fail "build watcher does not quarantine stable package signatures"
grep -Fq -- "--exclude='Dependencies/'" scripts/build-watch.sh || fail "build watcher does not preserve heavyweight Whisper dependencies"
grep -Fq 'Library/Caches/Shar/whisper' scripts/prepare_local_whisper.sh || fail "persistent Whisper cache path missing"
grep -Fq -- '--continue-at -' scripts/prepare_local_whisper.sh || fail "Whisper downloads are not resumable"
grep -Fq 'Recovering local Whisper model from previous build output' scripts/prepare_local_whisper.sh || fail "Whisper model previous-build recovery missing"
grep -Fq 'if version_gt "$current_version" "$version"; then' scripts/build-watch.sh || fail "build watcher does not skip archive versions older than current repository"
grep -Fq 'notarize_and_wait(){' scripts/build_macos_release.sh || fail "macOS resumable notarization helper missing"
grep -Fq 'notarytool submit "$artifact" --keychain-profile "$NOTARY_PROFILE" --output-format json' scripts/build_macos_release.sh || fail "macOS notarization is not submit-once JSON mode"
grep -Fq 'notarytool info "$submission_id"' scripts/build_macos_release.sh || fail "macOS notarization does not poll an existing submission ID"
grep -Fq 'notary_state=' scripts/build_macos_release.sh || fail "macOS notarization does not use zsh-safe notary_state variable"
! grep -Eq 'local .*([[:space:]]|^)status([[:space:]=]|$)' scripts/build_macos_release.sh || fail "macOS notarization reintroduced zsh read-only status variable"
grep -Fq 'SHAR_NOTARY_MAX_WAIT_SECONDS:-21600' scripts/build_macos_release.sh || fail "macOS notarization long outage recovery window missing"
grep -Fq 'SHAR_APPLE_NETWORK_ATTEMPTS' scripts/build_macos_release.sh scripts/check_macos_release_credentials.sh || fail "Apple network retry controls missing"
! grep -Fq 'notarytool submit "$NOTARY_ZIP" --keychain-profile "$NOTARY_PROFILE" --wait' scripts/build_macos_release.sh || fail "old duplicate-prone app submit --wait retry remains"
! grep -Fq 'notarytool submit "$DMG" --keychain-profile "$NOTARY_PROFILE" --wait' scripts/build_macos_release.sh || fail "old duplicate-prone DMG submit --wait retry remains"
