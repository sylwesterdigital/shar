...
grep -Fq 'forInterval: CMTime(seconds: 0.05' LocalWebShare/MediaSupport.swift || fail "iOS audio clock is not updating fast enough for Live spectrum"
grep -Fq 'let targetFPS = min(12.0' LocalWebShare/MediaPlayerView.swift || fail "iOS dense local spectrum analysis missing"
grep -Fq 'a[index] + (b[index] - a[index]) * mix' LocalWebShare/MediaPlayerView.swift || fail "iOS spectrum interpolation missing"
grep -Fq 'AudioCaptionChunker.makeChunks' LocalWebShare/MediaPlayerView.swift || fail "iOS long-audio caption chunking missing"
grep -Fq 'Try Apple online transcription' LocalWebShare/MediaPlayerView.swift || fail "iOS explicit Apple online caption fallback missing"
grep -Fq 'requiresOnDeviceRecognition = requiresOnDevice' LocalWebShare/MediaPlayerView.swift || fail "iOS caption privacy-first mode selector missing"
...
grep -Fq 'requiresOnDeviceRecognition = requiresOnDevice' LocalWebShare/MediaPlayerView.swift || fail "iOS captions must default through the privacy-first on-device/explicit-online selector"
